...
class InteractiveGitWrapper:
    ...
    def detect_new_remote_branches(self, remote: str) -> List[str]:
        """Detect new branches on remote that don't exist locally"""
        try:
            ...
            for remote_branch in remote_branches:
                ...
                if '/' in remote_branch:
                    ...
                    if branch_name not in local_branches:
                        new_branches.append(remote_branch)
    ...
    def interactive_sync(self):
        ...
        new_branches = self.detect_new_remote_branches(remote)
        if new_branches:
            self.print_info(f"Found {len(new_branches)} new remote branch(es):")
            for rb in new_branches[:5]:  # Show first 5
                print(f"  • {rb}")
            if len(new_branches) > 5:
                print(f"  ... and {len(new_branches) - 5} more")
            
            if self.confirm("Would you like to see all new branches?", False):
                self.interactive_show_new_branches(remote)
    ...
    def interactive_show_new_branches(self, remote: str):
        """Show and optionally checkout new remote branches"""
        new_branches = self.detect_new_remote_branches(remote)
        if not new_branches:
            self.print_info("No new remote branches found")
            return

        print(f"\n📋 New branches on {remote}:")
        for i, branch in enumerate(new_branches, 1):
            print(f"  {i}. {branch}")

        if self.confirm("\nCheckout one of these branches?", False):
            try:
                choice = int(input(f"Enter branch number (1-{len(new_branches)}): "))
                if 1 <= choice <= len(new_branches):
                    selected_branch = new_branches[choice - 1]
                    # Extract branch name without remote prefix
                    branch_name = selected_branch.split('/', 1)[1] if '/' in selected_branch else selected_branch
                    
                    self.print_working(f"Checking out {branch_name}...")
                    if self.run_git_command(['git', 'checkout', '-b', branch_name, selected_branch]):
                        self.print_success(f"Checked out new branch: {branch_name}")
                        self.add_to_history('branch', f"Checked out remote branch {branch_name}")
    ...
    def interactive_fetch(self):
        ...
        if self.run_git_command(['git', 'fetch', remote]):
            ...
            if new_branches:
                ...
                if self.confirm("Would you like to checkout one of these branches?", False):
                    self.interactive_show_new_branches(remote)
    ...
    def interactive_branch_menu(self):
        ...
        while True:
            ...
            options = [
                ...
                "Pull remote branch", "Delete branch", "Rename branch", "Back to main menu"
            ]
            ...
            handlers = {
                ...
                "Pull remote branch": self.interactive_pull_remote_branch,
    ...
    def interactive_pull_remote_branch(self):
        """Pull and checkout a remote branch"""
        self.clear_screen()
        print("🌿 Pull Remote Branch\n" + "=" * 25)
        ...
        remote = self.get_choice("Select remote:", remotes, default_remote)

        # Fetch latest
        self.print_working(f"Fetching from {remote}...")
        if not self.run_git_command(['git', 'fetch', remote], show_output=False):
            self.print_error("Failed to fetch from remote")
            input("Press Enter to continue...")
            return
        ...
        if not remote_branches:
            self.print_info(f"No branches found on {remote}")
            input("Press Enter to continue...")
            return

        # Get local branches to filter out existing ones
        local_result = subprocess.run(
            ['git', 'branch', '--format=%(refname:short)'],
            capture_output=True, text=True, check=True
        )
        ...
        # Show options: all remote branches or just new ones
        show_choice = self.get_choice(
            "Which branches to show?",
            ["Only new branches (not checked out locally)", "All remote branches"]
        )

        if "Only new" in show_choice:
            # Filter to show only new branches
            branches_to_show = []
            for rb in remote_branches:
                branch_name = rb.split('/', 1)[1] if '/' in rb else rb
                if branch_name not in local_branches:
                    branches_to_show.append(rb)
            ...
            branches_to_show = remote_branches

        print(f"\n📋 Available branches on {remote}:")
        for i, branch in enumerate(branches_to_show, 1):
            branch_name = branch.split('/', 1)[1] if '/' in branch else branch
            is_local = branch_name in local_branches
            marker = " (already local)" if is_local else " (new)"
            print(f"  {i}. {branch}{marker}")

        try:
            choice = int(input(f"\nEnter branch number to checkout (1-{len(branches_to_show)}, 0 to cancel): "))
            if choice == 0:
                return
            
            if 1 <= choice <= len(branches_to_show):
                selected_branch = branches_to_show[choice - 1]
                branch_name = selected_branch.split('/', 1)[1] if '/' in selected_branch else selected_branch
                
                # Check if branch already exists locally
                if branch_name in local_branches:
                    self.print_working(f"Switching to existing branch: {branch_name}")
                    if self.run_git_command(['git', 'checkout', branch_name]):
                        self.print_success(f"Switched to branch: {branch_name}")
                        
                        # Optionally pull latest changes
                        if self.confirm("Pull latest changes?", True):
                            if self.run_git_command(['git', 'pull', remote, branch_name]):
                                self.print_success("Pulled latest changes")
                else:
                    self.print_working(f"Checking out new branch: {branch_name}")
                    if self.run_git_command(['git', 'checkout', '-b', branch_name, selected_branch]):
                        self.print_success(f"Checked out new branch: {branch_name}")
                        self.add_to_history('branch', f"Checked out remote branch {branch_name}")
            else:
...
• Set default remote for operations
• Change remote URLs
• Fetch from remotes with new branch detection
• Multi-remote push capabilities
• Automatic detection of new remote branches
